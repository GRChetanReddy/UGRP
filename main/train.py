...
if __name__ == "__main__":
    ...
    if task == 'regression':
        result = 'Epoch\tTime(sec)\tLoss_train\tMAE_dev\tRMSE_dev\tMAE_test\tRMSE_test'
    ...
    for epoch in range(iteration):
        ...
        result = '\t'.join(map(str, [epoch, time, loss_train,
                                     prediction_dev_mae, prediction_dev_rmse, prediction_test_mae, prediction_test_rmse]))
        
        tester.save_result(result, file_result)
        torch.save(model.state_dict(), 'BestModel_ub_hex.pt')
        print(result)
