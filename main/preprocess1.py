...
def create_datasets(task, dataset, radius, device, arr): #arr should contain the pred version numbers for prediction
    ...
    datasets = []
    for i in arr:
        datasets.append(create_dataset('pentapepsmi'+str(i)+'_w.txt'))
        

    N_fingerprints = len(fingerprint_dict)

    return datasets, N_fingerprints
